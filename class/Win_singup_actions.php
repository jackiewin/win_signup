...
namespace XoopsModules\Win_singup_actions;

use XoopsModules\Tadtools\FormValidator;
use XoopsModules\Tadtools\Utility;

class Win_singup_actions
{
    //列出所有資料
    ...
    public static function create($id = '')
    {
        ...
        $op = empty($id) ? "win_singup_actions_store" : "win_singup_actions_update";
    ...
    public static function store()
    {
        ...
        $sql = "insert into `" . $xoopsDB->prefix("win_singup_actions") . "` (
        `欄位1`,
        `欄位2`,
        `欄位3`
        ) values(
    ...
    public static function update($id = '')
    {
        ...
        $sql = "update `" . $xoopsDB->prefix("win_singup_actions") . "` set
        `欄位1` = '{$欄位1值}',
        `欄位2` = '{$欄位2值}',
        `欄位3` = '{$欄位3值}'
        where `id` = '$id'";
    ...
    public static function destroy($id = '')
    {
        ...
        $sql = "delete from `" . $xoopsDB->prefix("win_singup_actions") . "`
        where `id` = '{$id}'";
        $xoopsDB->queryF($sql) or Utility::web_error($sql, __FILE__, __LINE__);
    }
    ...
    public static function get($id = '')
    {
        ...
        $sql = "select * from `" . $xoopsDB->prefix("win_singup_actions") . "`
        where `id` = '{$id}'";
    ...
    public static function get_all($auto_key = false)
    {
        ...
        $sql = "select * from `" . $xoopsDB->prefix("win_singup_actions") . "` where 1 ";
        $result = $xoopsDB->query($sql) or Utility::web_error($sql, __FILE__, __LINE__);
        $data_arr = [];
